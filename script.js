...
let tableEntries = JSON.parse(localStorage.getItem('expenseData')) || [];

// Function to save data to localStorage
function saveData() {
    localStorage.setItem('expenseData', JSON.stringify(tableEntries));
}
...
function addItem() {
    ...
    updateTable();
    saveData(); // Save the new state after adding an entry
    name.value = "";
    amount.value = 0;
}
...
function del(el) {
    ...
    saveData(); // Save the new state after deletion
    updateSummary();
}
